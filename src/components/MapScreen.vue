      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            const res = searchService.getLiblaries(`139.7673068` + "," + `35.680959`);
            res.then((value) => {
  ...
  <template>
    <div class="w-full h-full z-0">
      ...
      <div @click="onClickList" class="z-10 bottom-16 right-4 absolute w-16 h-16 bg-primary text-lg font-bold rounded-full container shadow-md shadow-gray-500 py-4 px-4">
        <img src="../assets/icon/list.png"/>
      </div>

      <ol-map :loadTilesWhileAnimating="true" :loadTilesWhileInteracting="true" style="height:100%" v-if="showMap">
      ...
      <div class="mt-4" v-else>
        <div v-for="library in libraries" :key="library">
          <div class="p-3">
            ...
            <p class="w-fit font-bold text-xs mb-2 bg-slate-200">{{ library.city }}</p>
